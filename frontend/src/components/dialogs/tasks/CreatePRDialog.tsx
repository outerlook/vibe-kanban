...
const CreatePRDialogImpl = NiceModal.create<CreatePRDialogProps>(
  ({ attempt, task, repoId, targetBranch }) => {
    ...
    useEffect(() => {
      ...
      // Reset all form state when dialog opens to prevent stale data flash
      setPrBaseBranch('');
      setIsDraft(false);
      ...
      // Initialize form values from task (only when auth is loaded)
      if (isLoaded) {
        setPrTitle(`${task.title} (vibe-kanban)`);
        setPrBody(task.description || '');
      }
    }, [modal.visible, isLoaded, task, config?.pr_auto_description_enabled]);
    ...
    const handleConfirmCreatePR = useCallback(async () => {
      ...
      if (result.success) {
        // Form state will be reset by useEffect when dialog reopens
        modal.hide();
        return;
      }
